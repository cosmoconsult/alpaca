...
1. [Sign up and create an organization](../github/create-org.md)
1. [Set-up VS Code and the COSMO Alpaca Extension](setup-vsce.md)
...
1. [Create a project](../github/create-project.md) (optional for GitHub, only if you want a multi-project repository)
2. [Create repository, AL App and pipelines](../github/create-app.md)
3. [Open your project/repository in the browser](../shared/open-stuff.md) and create a new requirement that you'll be working on next
...
1. [Clone your app repository](../azure-devops/clone-repo.md) to your local machine for development. Open the workspace.
...
1. While the pipeline runs, take the time to check your [WorkItem](../shared/open-stuff.md). You can observe that everything is linked: From the development branch, over the commit on to the Pull Request. Full end-to-end tracking of changes now is a breeze.
1. If the pipeline succeeded, either [delete](../azure-devops/delete-container.md) or [stop](../azure-devops/startstop-container.md) your container to free up resources for other users.
...
1. Now at the latest also the developer would [delete the development container](../azure-devops/delete-container.md) if it was just stopped before.
-->

## Deploying your app

There comes a time in your project where you need to deploy the current state of the project. The default way would be to use Release Pipelines to (continuously) deploy changes to a test, QA and production environment. You can set up release pipelines to deploy to SaaS or OnPrem environments with a single click via [VS Code](../azure-devops/create-release-pipeline.md). Afterwards you need to [configure the connection](../azure-devops/create-release-pipeline.md#configuration) to your environment.
...
- Quickly want to set up a container for a specific version and country that is not related to a project? You can use [Standalone Containers](../azure-devops/create-ondemand-container.md) to create those environments with a simple wizard.

- If you work in many projects you may get overwhelmed by the long Azure DevOps organization list. Declutter your workspace and configure your [favorite organizations](../azure-devops/favorite-orgs.md)

- You got a project that wasn't created with COSMO Alpaca? The [onboarding function](../azure-devops/onboard-project.md) is able to make most of your project ready for it.

- Do you need to execute some custom logic for all the development, test or demo containers in your project to e.g. add users or install another dependency not covered by the default configuration? You can set up [custom scripts](../azure-devops/setup-custom-scripts.md) that are automatically executed on container start.

- If you need to extend the default pipelines, you can use the available [Pipeline Hooks](../azure-devops/pipeline-variables.md#pipeline-setup-hooks)

- Administrators can [set up configurations](../admin/index.md) which affect all users of COSMO Alpaca, independent of the project:
