---
    title: Setup cosmo.json Config
    description: Setup cosmo.json Config
---
...
- **COSMO-only**: If the default Cronus license doesn't work for you, you can use a country-specific development license variable. Unfortunately we can only provide updated licenses for the latest BC versions, but to avoid updating the license in your cosmo.json repeatedly, you can now use the new "latest" license secrets. Those secrets are named after the following format: `dbclic-bc-<language>` *(.bclicense)* or `devlic-bc-<language>` *(.flf)*. e.g.:
...
|`"codeCopsTest"`            |string[]  |optional|Defines the array of the AL Analyzer(s) used for compiling for the test project|
